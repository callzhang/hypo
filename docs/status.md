...
- **SMS Auto-Sync Feature** ✅: Implemented automatic SMS copying and syncing
  ...
  - Runtime permission request with UI status display in Settings
  - Periodic permission status monitoring (every 2 seconds)
  - User-friendly permission management with grant button
  - Android 10+ limitation warnings displayed to users
  - Documented in `docs/features/sms_sync.md` and `docs/technical.md`

- **Build System Improvements** ✅: Always build apps on code changes
  - Modified `build-macos.sh` to always force rebuild (touch Package.swift)
