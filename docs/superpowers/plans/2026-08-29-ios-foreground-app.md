...
## 补上"长时间后台后恢复"的测试（2026-09-01）

真机每天都在做、而测试完全没覆盖的一件事:iOS 挂起后台的 app,回来后发现和连接必须自己恢复。现有几处 `press(.home)` 只背景两秒,是为搬运剪贴板准备的,不足以触发挂起。

新测试背景 30 秒再回来,验证界面仍然响应、状态行仍然报得出已知状态,并且**背景前看得见的设备在给它一个浏览周期后回来**。

### 第一版是空过的

它通过了,但当时网络上一个设备都没有,于是那段"设备恢复"的断言被 `guard peersBefore > 0 else { return }` **静默跳过**——只验了"界面还在",也就是弱的那一半,而且报告里看不出来。改成 `XCTSkip` 并写明跳过了什么。
...
| 观察 | 排除了什么 |
|---|---|
| 自 10:30 起 `Discovery/` 与 `TransportManager.swift` **无任何提交**,而 11:01 时发现还正常 | 代码回归 |
| **全新创建的模拟器同样发现不了** | 那台模拟器的状态 |
...
## 模拟器发现不到设备:第四次诊断才查到证据（2026-09-01）

同一个现象我给出过四次判断,前三次都是猜的:

| # | 判断 | 结果 |
|---|---|---|
| 1 | 模拟器 mDNS 卡死,需要 `sudo killall -HUP mDNSResponder` | 错。新建模拟器、重启 CoreSimulator 都无效 |
| 2 | Mac 换了网络 | 部分对——解释了手机不可见,不解释本机 harness 也看不到 |
| 3 | `didNotSearch` 里只 `services.removeAll()`,失败被静默吞掉 | 错。合并 main 的重试修复后仍是零,且**没有任何重试日志**,说明浏览根本没失败 |
| 4 | **Tailscale 抢走了组播路由** | **有证据** |

第四次的证据在路由表里:

```
224.0.0/4    link#12    UmCS     en0    !
224.0.0/4    link#21    UmCSI    utun5
```

组播段有两条竞争路由,一条走 en0(带拒绝标记),一条走 Tailscale 的 `utun5`。mDNS 完全依赖组播,所以模拟器的组播被送进隧道,到不了局域网。宿主的 `dns-sd` 仍可用,是因为 mDNSResponder 自己管理接口绑定,不完全走路由表。

**验证方法**:临时退出 Tailscale,重新观察模拟器是否能发现。

### 这一节真正的教训

前三次每一次都"看起来合理"就停了,而且**每一次都写进了文档**,包括一条不必要的 sudo 建议。一条 `netstat -rn` 就在手边,四次才想起来看。

排查网络问题时,**先看路由和接口,再看应用层**。这个顺序反过来,就会像这次一样:换模拟器、重启服务、读源码、合并别人的修复——全都做过一遍,才回到最基础的那张表。
