...
- **BonjourPublisher** (`Utilities/BonjourPublisher.swift`): Publishes `_hypo._tcp` with TXT payload `{ version, fingerprint_sha256, protocols, device_id, public_key }`. The service name is set to `DeviceIdentity.deviceName` (sanitized to remove `.local` suffix for Android compatibility). It tracks the currently advertised endpoint and restarts advertising when configuration changes (e.g., port update).
...
- **Handshake**: Client sends `X-Device-Id` and `X-Device-Platform` headers during WebSocket upgrade to identify itself to the server immediately.
