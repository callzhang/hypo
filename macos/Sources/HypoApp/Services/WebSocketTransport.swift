        ...
        task.receive { [weak self] result in
            ...
            case .failure(let error):
                ...
                if self.configuration.environment == "cloud" || self.configuration.url.scheme == "wss" {
                    ...
                    let reconnectMsg = "🔄 [WebSocketTransport] Cloud connection reset (retry \(self.receiveRetryCount)), reconnecting after \(Int(backoff))s backoff...\n"
                    logger.info("🔄 [WebSocketTransport] Cloud connection reset (retry \(self.receiveRetryCount)), reconnecting after \(Int(backoff))s backoff...")
                    fflush(stdout)
