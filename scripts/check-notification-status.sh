...
# Get app PID
APP_PID=$(adb_cmd shell pidof -s com.hypo.clipboard.debug 2>/dev/null || echo "")
if [ -z "$APP_PID" ]; then
...
adb_cmd logcat -d --pid="$APP_PID" | grep -vE "MIUIInput|SKIA|VRI|RenderThread" | grep -E "ClipboardSyncService|Notification|notification" | tail -30
echo ""

# Check for notification channel creation
echo "=== Notification Channel Status ==="
CHANNEL_LOG=$(adb_cmd logcat -d --pid="$APP_PID" | grep -vE "MIUIInput|SKIA|VRI|RenderThread" | grep "Notification channel created" | tail -1)
...
FOREGROUND_LOG=$(adb_cmd logcat -d --pid="$APP_PID" | grep -vE "MIUIInput|SKIA|VRI|RenderThread" | grep -E "Foreground service|startForeground" | tail -3)
...
UPDATE_LOG=$(adb_cmd logcat -d --pid="$APP_PID" | grep -vE "MIUIInput|SKIA|VRI|RenderThread" | grep "Notification updated" | tail -5)
...
ERROR_LOG=$(adb_cmd logcat -d --pid="$APP_PID" | grep -vE "MIUIInput|SKIA|VRI|RenderThread" | grep -E "Notification.*[Ee]rror|Failed.*notification|CRITICAL.*notification" | tail -10)
if [ -n "$ERROR_LOG" ]; then
    echo "$ERROR_LOG"
else
...
OBSERVE_LOG=$(adb_cmd logcat -d --pid="$APP_PID" | grep -vE "MIUIInput|SKIA|VRI|RenderThread" | grep -E "observe.*latest|Latest item changed" | tail -5)
