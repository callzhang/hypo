#!/bin/bash
# Comprehensive pairing debug script - monitors both Android and macOS logs
...
# Step 1: Stop existing macOS app
...
# Step 3: Verify WebSocket server is listening
...
log_info ""
log_info "═══════════════════════════════════════════════════════════"
log_info "🔍 PAIRING DEBUG MONITOR - Ready for pairing attempt"
log_info "═══════════════════════════════════════════════════════════"
log_info ""
...
# Monitor Android logs in background
(
    "$ANDROID_SDK_ROOT/platform-tools/adb" logcat | grep -E "LanPairingViewModel|LanWebSocketClient|pairing|Pairing|WebSocket|Connection|challenge|ACK|sendRawJson|onOpen|onFailure" | while IFS= read -r line; do
...
# Monitor macOS logs
log stream --predicate 'process == "HypoMenuBar"' --level debug 2>/dev/null | grep -E "pairing|challenge|Connection|Received|WebSocket|ACK|❌|✅|⚠️|📥|📤|🔌|📋|🔔" | while IFS= read -r line; do
