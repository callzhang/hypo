...
public final class KeychainDeviceKeyProvider: DeviceKeyProviding {
    ...
    /// Check if a key exists for a device without throwing an error
    public func hasKey(for deviceId: String) -> Bool {
        // FileBasedKeyStore.load returns nil if key doesn't exist, doesn't throw
        return (try? keyStore.load(for: deviceId)) != nil
    }
}
