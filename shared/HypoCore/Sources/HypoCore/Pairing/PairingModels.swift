...
public struct PairingChallengeMessage: Codable, Equatable, Sendable {
    ...
    /// Declaring `init(from:)` below suppresses the synthesised memberwise
    /// init, and until now nothing in Swift ever built one of these — only
    /// Android and the .NET harness sent challenges, Swift only received them.
    public init(
        challengeId: UUID,
