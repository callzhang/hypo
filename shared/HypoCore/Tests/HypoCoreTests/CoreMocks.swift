...
// NOTE: This file duplicates a handful of small test doubles that are also defined in
// macos/Tests/HypoAppTests/MockSupport.swift (MockNotificationController, MockBonjourPublisher(+State),
// MockTransportProvider(+State), InMemoryLanDiscoveryCache, makeWebSocketServer). HypoAppTests and
// HypoCoreTests are separate SwiftPM targets with no shared test-utility target between them, so the
// options were duplicate these five small doubles or introduce a third library target just to share
// test code. Duplication was judged the smaller cost.
